...
# 
from llama3.training import MultiTrainer
...
def multitrain_llama3_model(args : argparse.Namespace):
    specification = Specification(args)
    MultiTrainer(specification).train(from_checkpoint=args.from_checkpoint)


def add_subparser(subparsers : argparse._SubParsersAction):
    parser = subparsers.add_parser('multitrain', help='multitrain LLAMA3 model')
    # corpus 
    ...
    group.add_argument('--max_norm', default=None, type=float, 
                       help='max norm for clipping gradients')
    ...
    group.add_argument("--prefetching", type=positive_integer, default=1, 
                       help='prefetching for CPU bound use-cases. by default it prefetches one layer. ')
    # set defaults 
    parser.set_defaults(func=multitrain_llama3_model)
