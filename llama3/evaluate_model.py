...
# 
from llama3.evaluation import Evaluator
...
def eval_llama3_model(args : argparse.Namespace):
    specification = Specification(args)
    Evaluator(specification).evaluate(from_checkpoint=args.from_checkpoint)


def add_subparser(subparsers : argparse._SubParsersAction):
    parser = subparsers.add_parser('evaluate', help='evaluate LLAMA3 model')
    # corpus 
    ...
    group.add_argument('--total_steps', default=-1, type=nonzero_integer,
                       help='number of total eval steps')
    ...
    # set defaults 
    parser.set_defaults(func=eval_llama3_model)
